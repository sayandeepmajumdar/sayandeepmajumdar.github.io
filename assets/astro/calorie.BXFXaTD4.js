function R(t){t.innerHTML=`
  ...
  `}function N({standard:t,gender:m,age:i,heightCm:n,weightKg:l,activity:h,goalKcal:x,macroPreset:w}){if(!i||!n||!l||i<=0||n<=0||l<=0)return null;let o=0;t==="icmr"?m==="male"?i<30?o=14.5*l+645:i<=60?o=10.9*l+833:o=12.8*l+463:i<30?o=14*l+471:i<=60?o=8.3*l+788:o=10*l+565:m==="male"?o=10*l+6.25*n-5*i+5:o=10*l+6.25*n-5*i-161;const k=parseFloat(h)||1.55,M=Math.round(o*k),S=parseInt(x,10)||0,b=Math.max(m==="female"?1200:1500,Math.round(M+S)),_={balanced:{carbPct:50,protPct:20,fatPct:30,label:"50% Carb · 20% Prot · 30% Fat"},high_protein:{carbPct:40,protPct:30,fatPct:30,label:"40% Carb · 30% Prot · 30% Fat"},low_carb:{carbPct:20,protPct:35,fatPct:45,label:"20% Carb · 35% Prot · 45% Fat"},indian_trad:{carbPct:60,protPct:15,fatPct:25,label:"60% Carb · 15% Prot · 25% Fat"}},c=_[w]||_.balanced,C=Math.round(b*c.carbPct/100),P=Math.round(C/4),f=Math.round(b*c.protPct/100),g=Math.round(f/4),v=Math.round(b*c.fatPct/100),I=Math.round(v/9),y=(g/l).toFixed(1),s=((l*35+(k>1.4?500:0))/1e3).toFixed(1),a=n/100,r=(l/(a*a)).toFixed(1),d=parseFloat(r);let u="",p="";d<18.5?(u="Underweight",p="under"):d<23?(u="Normal Weight",p="normal"):d<25?(u="Overweight",p="over"):(u="Obese",p="obese");let e="";return d<18.5?e="Underweight":d<25?e="Normal":d<30?e="Overweight":e="Obese",{bmr:Math.round(o),tdee:M,targetKcal:b,delta:S,waterLitres:s,split:c,macros:{carb:{grams:P,kcal:C,pct:c.carbPct},prot:{grams:g,kcal:f,pct:c.protPct,perKg:y},fat:{grams:I,kcal:v,pct:c.fatPct}},bmi:r,asianCategory:u,asianBadge:p,whoCategory:e}}function z(t){const m=t.querySelector("#cal-std-seg"),i=t.querySelector("#cal-gender-seg"),n=t.querySelector("#cal-unit-seg"),l=t.querySelector("#cal-metric-wrap"),h=t.querySelector("#cal-imperial-wrap"),x=t.querySelector("#cal-calc-btn"),w=t.querySelector("#cal-result"),o=t.querySelector("#cal-target-val"),k=t.querySelector("#cal-badges-wrap"),M=t.querySelector("#cal-stats"),S=t.querySelector("#bar-carb"),F=t.querySelector("#bar-prot"),b=t.querySelector("#bar-fat"),_=t.querySelector("#cal-macro-ratio-label"),c=t.querySelector("#cal-macro-cards"),C=t.querySelector("#cal-bmi-num"),P=t.querySelector("#cal-bmi-badges"),f=t.querySelector("#cal-bmi-note"),g=t.querySelector("#cal-burn-insight");if(!x)return;let v="icmr",I="male",y="metric";m&&m.addEventListener("click",s=>{const a=s.target.closest("[data-std]");a&&(v=a.dataset.std,m.querySelectorAll(".segment__btn").forEach(r=>r.classList.remove("active")),a.classList.add("active"))}),i&&i.addEventListener("click",s=>{const a=s.target.closest("[data-gender]");a&&(I=a.dataset.gender,i.querySelectorAll(".segment__btn").forEach(r=>r.classList.remove("active")),a.classList.add("active"))}),n&&n.addEventListener("click",s=>{const a=s.target.closest("[data-unit]");a&&(y=a.dataset.unit,n.querySelectorAll(".segment__btn").forEach(r=>r.classList.remove("active")),a.classList.add("active"),y==="metric"?(l.style.display="",h.style.display="none"):(l.style.display="none",h.style.display=""))}),x.addEventListener("click",()=>{let s,a,r;if(y==="metric")s=parseInt(t.querySelector("#cal-age").value,10),a=parseFloat(t.querySelector("#cal-height-cm").value),r=parseFloat(t.querySelector("#cal-weight-kg").value);else{s=parseInt(t.querySelector("#cal-age-imp").value,10);const H=parseFloat(t.querySelector("#cal-weight-lbs").value),T=parseFloat(t.querySelector("#cal-height-ft").value)||0,E=parseFloat(t.querySelector("#cal-height-in").value)||0;r=H*.453592,a=(T*12+E)*2.54}const d=t.querySelector("#cal-activity").value,u=t.querySelector("#cal-goal").value,p=t.querySelector("#cal-macro-preset").value,e=N({standard:v,gender:I,age:s,heightCm:a,weightKg:r,activity:d,goalKcal:u,macroPreset:p});if(!e){o.textContent="Invalid input",w.classList.remove("result-box--hidden");return}w.classList.remove("result-box--hidden"),o.textContent=e.targetKcal.toLocaleString("en-IN");const q=v==="icmr"?"🇮🇳 ICMR-NIN 2020 RDA":"🌐 Mifflin-St Jeor";let L="Maintain Weight";e.delta<0?L=`${Math.abs(e.delta)} kcal Deficit`:e.delta>0&&(L=`+${e.delta} kcal Surplus`),k.innerHTML=`
      <span class="bmi-badge bmi-badge--normal" style="margin-top:0">${q}</span>
      <span class="bmi-badge ${e.delta<0?"bmi-badge--over":"bmi-badge--normal"}" style="margin-top:0">${L}</span>
    `;const $=e.delta>0?`+${e.delta}`:e.delta<0?`${e.delta}`:"0";M.innerHTML=`
      <div class="result-stat">
        <div class="result-stat__label">Basal Metabolic Rate (BMR)</div>
        <div class="result-stat__value">${e.bmr.toLocaleString()} kcal</div>
      </div>
      <div class="result-stat">
        <div class="result-stat__label">Maintenance (TDEE)</div>
        <div class="result-stat__value">${e.tdee.toLocaleString()} kcal</div>
      </div>
      <div class="result-stat">
        <div class="result-stat__label">Daily Adjustment</div>
        <div class="result-stat__value">${$} kcal</div>
      </div>
      <div class="result-stat">
        <div class="result-stat__label">Hydration Target</div>
        <div class="result-stat__value">~${e.waterLitres} L / day</div>
      </div>
    `,S.style.width=e.split.carbPct+"%",F.style.width=e.split.protPct+"%",b.style.width=e.split.fatPct+"%",_.textContent=e.split.label,c.innerHTML=`
      <div style="background:var(--color-surface-2);border-radius:8px;padding:.75rem;border-left:3px solid #f59e0b">
        ...
        <div style="font-family:var(--font-mono);font-size:1.3rem;font-weight:700;color:var(--color-text);margin-top:.2rem">${e.macros.carb.grams}g</div>
        <div style="font-size:.78rem;color:var(--color-text-muted);margin-top:.15rem">${e.macros.carb.kcal} kcal (${e.macros.carb.pct}%)</div>
      </div>
      ...
      <div style="background:var(--color-surface-2);border-radius:8px;padding:.75rem;border-left:3px solid #3b82f6">
        ...
        <div style="font-family:var(--font-mono);font-size:1.3rem;font-weight:700;color:var(--color-text);margin-top:.2rem">${e.macros.prot.grams}g</div>
        <div style="font-size:.78rem;color:var(--color-text-muted);margin-top:.15rem">${e.macros.prot.kcal} kcal (${e.macros.prot.perKg}g/kg)</div>
      </div>
      ...
      <div style="background:var(--color-surface-2);border-radius:8px;padding:.75rem;border-left:3px solid #10b981">
        ...
        <div style="font-family:var(--font-mono);font-size:1.3rem;font-weight:700;color:var(--color-text);margin-top:.2rem">${e.macros.fat.grams}g</div>
        <div style="font-size:.78rem;color:var(--color-text-muted);margin-top:.15rem">${e.macros.fat.kcal} kcal (${e.macros.fat.pct}%)</div>
      </div>
    `,C.textContent=`BMI ${e.bmi}`,P.innerHTML=`
      <span class="bmi-badge bmi-badge--${e.asianBadge}" style="margin-top:0">Asian: ${e.asianCategory}</span>
      <span class="bmi-badge bmi-badge--normal" style="margin-top:0">WHO: ${e.whoCategory}</span>
    `,parseFloat(e.bmi)>=23&&parseFloat(e.bmi)<25?f.innerHTML='⚠️ <strong>Attention:</strong> While WHO classifies BMI 23.0–24.9 as "Normal", Indian Health Ministry &amp; ICMR guidelines classify BMI ≥ 23 as <strong>Overweight</strong> due to higher visceral fat prevalence in South Asian populations.':f.textContent=`Asian-Indian cutoff: Normal (18.5–22.9), Overweight (23–24.9), Obese (≥25). Reference protein RDA for ${r.toFixed(0)} kg is ${(r*.83).toFixed(1)}g/day.`;const A=Math.round(260/(r*3.5)*60),B=(Math.abs(e.delta)/260).toFixed(1);e.delta<0?g.innerHTML=`💡 <strong>Indian Food Context:</strong> Your daily calorie deficit of <strong>${Math.abs(e.delta)} kcal</strong> is roughly equal to skipping <strong>${B} Punjabi Samosas</strong> (260 kcal each) or burning off <strong>${A} minutes</strong> of brisk walking for your current weight.`:e.delta>0?g.innerHTML=`💡 <strong>Indian Food Context:</strong> Your target calorie surplus of <strong>+${e.delta} kcal</strong> provides the clean energy needed for lean tissue growth. Aim for whole protein sources like Paneer, Dal, Soya Chunks, Greek Curd, or Eggs.`:g.innerHTML=`💡 <strong>Indian Food Context:</strong> Eating at your maintenance level of <strong>${e.tdee.toLocaleString()} kcal</strong> preserves your metabolic rate and sustains lean muscle mass over the long term.`})}export{z as bindEvents,N as calculate,R as render};
