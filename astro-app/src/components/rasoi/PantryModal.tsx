...
import { X, ChefHat, Check, Sparkles } from 'lucide-react';
import { RECIPES } from '../../data/rasoi/recipes';
import type { PantryMatchResult } from '../../types/rasoi';
  ...
  return (
    <>
      <button
        ...
        className={
          className ||
          'flex items-center gap-2.5 px-6 py-3.5 rounded-full bg-amber-500 hover:bg-amber-400 text-stone-950 font-extrabold text-sm shadow-md hover:scale-105 active:scale-95 transition-all'
        }
      >
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 md:p-6">
          ...
          {/* Modal Dialog */}
          <div className="relative w-full max-w-4xl max-h-[90vh] bg-[#faf7f2] rounded-3xl shadow-2xl border border-stone-200 overflow-hidden flex flex-col z-10">
            {/* Header */}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                    {matchedResults.slice(0, 9).map(({ recipe, matchScore, missingIngredients }) => (
                      <a
                        ...
                        className="group bg-white rounded-2xl border border-stone-200 overflow-hidden shadow-2xs hover:shadow-lg hover:-translate-y-1 transition-all flex flex-col justify-between"
                      >
