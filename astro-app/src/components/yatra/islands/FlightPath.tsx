...
export const FlightPath: React.FC<FlightPathProps> = ({
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      // Initialize laser dash array once total length is known
      if (laserPathRef.current && !laserPathRef.current.style.strokeDasharray) {
        laserPathRef.current.style.strokeDasharray = `${totalLength}`;
        laserPathRef.current.style.strokeDashoffset = `${totalLength}`;
      }
      ...
      // Update laser path reveal
      if (laserPathRef.current) {
        laserPathRef.current.style.strokeDashoffset = `${Math.max(0, totalLength * (1 - eased))}`;
      }
  ...
  return (
    <g ref={groupRef} className="flight-path-group pointer-events-none">
      ...
      {/* Animated glowing laser flight path (Direct RAF strokeDashoffset animation) */}
      <path
        ref={laserPathRef}
        d={pathString}
        fill="none"
