...
export const DestinationModal: React.FC<DestinationModalProps> = ({
  ...
  const handleToggleSave = () => {
    const updated = toggleSavedId(destination.id);
    setSavedIds(updated);
  };

  const handleShare = () => {
    const url = `${window.location.origin}/yatra/destination/${destination.id}/`;
    if (navigator.share) {
      navigator
        .share({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 overflow-y-auto animate-fade-in">
      ...
      {/* Modal Card */}
      <div
        className="relative w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-3xl glass-panel shadow-2xl border border-white/15 text-stone-100 z-10 scrollbar-none transform transition-transform duration-300 scale-100"
      >
        {/* Header Hero Image */}
        <div className="relative h-64 sm:h-80 w-full overflow-hidden bg-stone-900">
          ...
          <div className="absolute top-4 right-4 flex items-center gap-2 z-10">
            <a
              href={`/yatra/destination/${destination.id}/`}
              title="Open Dedicated Page"
              className="p-2.5 rounded-full bg-black/50 text-white hover:bg-black/70 backdrop-blur-md transition-colors inline-flex items-center justify-center"
            >
