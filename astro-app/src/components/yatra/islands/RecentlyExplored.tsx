...
import { getRecentIds, RECENT_STORAGE_KEY } from '../../../utils/yatraSaved';

export const RecentlyExplored: React.FC = () => {
  const [recentIds, setRecentIds] = useState<string[]>([]);

  useEffect(() => {
    setRecentIds(getRecentIds());
    const handleStorage = () => setRecentIds(getRecentIds());
      ...
      <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-none snap-x">
        {recentDestinations.map((dest) => (
          <a
            ...
            className="flex-shrink-0 snap-start flex items-center gap-3 p-2.5 pr-4 rounded-2xl glass-card border border-white/5 hover:border-amber-500/30 cursor-pointer transition-all active:scale-95 group text-decoration-none"
          >
