...
interface ToolzyLogoProps {
  size?: 'sm' | 'md' | 'lg' | 'hero';
  showIcon?: boolean;
  className?: string;
}

export const ToolzyLogo: React.FC<ToolzyLogoProps> = ({
  size = 'md',
  showIcon = true,
  className = '',
}) => {
  ...
  return (
    <div className={`inline-flex items-center gap-2.5 sm:gap-3 select-none ${className}`}>
      ...
      {/* Google-Style Colored Lettering for Toolzy */}
      <span className={`${sizeClasses[size]} inline-flex font-sans`}>
        <span className="text-[#4285F4] dark:text-[#5B95F5] transition-colors">T</span>
        <span className="text-[#EA4335] dark:text-[#F06A5D] transition-colors">o</span>
        <span className="text-[#FBBC05] dark:text-[#FCD04B] transition-colors">o</span>
        <span className="text-[#4285F4] dark:text-[#5B95F5] transition-colors">l</span>
        <span className="text-[#34A853] dark:text-[#57B972] transition-colors">z</span>
        <span className="text-[#EA4335] dark:text-[#F06A5D] transition-colors">y</span>
      </span>
    </div>
  );
};
...
export const ForgeKitLogo = ToolzyLogo;
